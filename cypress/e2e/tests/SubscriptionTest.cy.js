...
describe("Login as New user and Buy Subscription", { tags: ['@Login', '@regression'] }, () => {
    ...
    it("should login successfully and go to subscription page", {tags: '@smoke'}, function () {
        LoginPage
        ...
        cy.wait(3000);
        SubscriptionFlow.clickonProfile.click();
        SubscriptionFlow.clickonSUbscription.eq(3).should('have.text','Subscription').click();
        AccountPage.subscriptionPageTitle
            .should('contain.text', 'Amplify Your Imprint');
    
        // Intercept both the request to b.stripecdn.com and billing.stripe.com
        cy.intercept('GET', '**/b.stripecdn.com/**').as('cdnRedirect');
        cy.intercept('GET', '**billing.stripe.com/**').as('stripeRedirect');
    
        // Click the "View Billing Portal" button
        cy.contains('button', 'View Billing Portal').should('be.visible').click();
        ...
        cy.wait('@cdnRedirect').then((interception) => {
            ...
            cy.log('Intercepted CDN Request:', interception.request.url);
            expect(interception.request.url).to.include('b.stripecdn.com');
            expect(interception.request.url).to.include('billing.stripe.com'); // Confirm that it includes billing.stripe.com as a reference
        });

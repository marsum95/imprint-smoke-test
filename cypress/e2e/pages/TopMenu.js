...
class TopMenu extends BasePage{
    ...
    get news() { return cy.get('[style="opacity:1;order:1"] > .ant-menu-title-content > :nth-child(1) > a')}//click on News icon 
    get entertainment() { return cy.get('[style="opacity:1;order:2"] > .ant-menu-title-content')}//click on Entertainment icon
    get notification() { return cy.get('.icon-notification')}//click on Notification icon 
    get message() { return cy.get('.icon-messanger')}//click on Messages icon 
    get analytical() {return cy.get('.icon-value-profile')}
    ...
    newsButton() {
        ...
        cy.get('.general-news_Header__0glk_')
            .should('contain.text', 'General News')

        // check for the content should be present
        cy.get('.general-news_GeneralNewsContainer__SW7B_ > :nth-child(1)').should('exist');
        cy.get('.general-news_GeneralNewsContainer__SW7B_ > :nth-child(1)').should('be.visible');
    }
    ...
    notificationButton() {
        ...
        cy.get('.ant-list-item').each(($el) => {
            ...
            if ($el.text().includes('Rudi') || $el.text().includes('SystemModeration')) {
    ...
    analyticalButton(){
        ...
        cy.get('.ant-tabs-tab-active')
            .should('have.text', 'Value Profile')  
    }
